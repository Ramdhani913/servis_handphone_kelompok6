    ...
    <table class="table table-hover text-center align-middle">
        ...
        <tbody>
            @forelse($handphones as $phone)
                <tr>
                    <td><img src="{{ asset('storage/' . $phone->image) }}" class="rounded-circle" width="40"
                            height="40"></td>
                    <td>{{ $phone->brand }}</td>
                    <td>{{ $phone->model }}</td>
                    <td>
                        <span
                            class="status-toggle fw-bold {{ $phone->is_active == 'active' ? 'text-success' : 'text-danger' }}"
                            style="cursor:pointer" data-id="{{ $phone->id }}">
                            {{ $phone->is_active }}
                        </span>
                    </td>
                    <td>
                        ...
                        <form action="{{ route('handphones.destroy', $phone->id) }}" method="POST" class="d-inline">
                            ...
                            <button class="btn btn-delete btn-sm" type="submit"
                                onclick="return confirm('Yakin hapus Model ini?');">Delete</button>
                        </form>
